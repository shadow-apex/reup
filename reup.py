...
class ReupApp(tk.Tk):
    ...
    def _translate_srt_google_now(self, segments):
        ...
        # ✅ TỐI ƯU: phụ đề hay có nhiều dòng LẶP LẠI y hệt nhau (điệp khúc,
        # câu cửa miệng...). Trước đây mỗi dòng đều gọi Google riêng dù nội
        # dung giống hệt dòng đã dịch trước đó -> lãng phí request, dễ bị
        # giới hạn tốc độ hơn. Giờ gom các dòng GIỐNG HỆT NHAU lại, chỉ dịch
        # 1 lần rồi dùng chung kết quả cho tất cả các dòng trùng.
        text_to_indices = {}
        for i in work_idx:
            t = segments[i]["text"].strip()
            text_to_indices.setdefault(t, []).append(i)
        unique_reps = [idxs[0] for idxs in text_to_indices.values()]
        dup_count = len(work_idx) - len(unique_reps)
        if dup_count:
            self._log(f"   ♻️ Có {dup_count} dòng trùng lặp nội dung — chỉ dịch 1 lần, dùng lại cho các dòng còn lại.")

        # ── Lượt 1: dịch tuần tự-nhẹ (2 luồng) để giảm nguy cơ bị chặn ──
        _run_pass(unique_reps, workers=2, label="lượt 1")

        # ── Lượt 2: chỉ dịch lại đúng những dòng còn thất bại, chạy đơn luồng ──
        failed_idx = [i for i in unique_reps if not ok_flags[i]]
        if failed_idx:
            self._log(f"   🔁 Có {len(failed_idx)} dòng lỗi ở lượt 1 — thử dịch lại lần nữa…")
            time.sleep(2)
            _run_pass(failed_idx, workers=1, label="lượt 2")

        # ── Áp kết quả từ dòng đại diện sang các dòng trùng lặp còn lại ──
        for idxs in text_to_indices.values():
            rep = idxs[0]
            for i in idxs[1:]:
                results[i] = results[rep]
                ok_flags[i] = ok_flags[rep]
    ...
    async def _tts_segments_synced(self, segments, out_audio, tdir):
        ...
        results = [None] * total_lines  # giữ đúng thứ tự dù chạy song song
        failed = []
        done_count = 0
        # ✅ TỐI ƯU: trước đây gọi edge-tts TUẦN TỰ từng dòng một (dòng sau
        # phải đợi dòng trước xong hẳn) -> với vài trăm dòng rất chậm, đứng
        # màn hình lâu. Giờ chạy TỐI ĐA `_TTS_CONCURRENCY` dòng cùng lúc
        # bằng asyncio.Semaphore, nhanh hơn ~3-4 lần, vẫn giữ retry/backoff
        # + bỏ qua dòng lỗi như cũ, và không dồn dập quá mức để tránh bị
        # Microsoft giới hạn tốc độ (throttle).
        _TTS_CONCURRENCY = 4
        sem = asyncio.Semaphore(_TTS_CONCURRENCY)
        log_lock = asyncio.Lock()

        async def _one(i, seg):
            nonlocal done_count
            ...
            async with sem:
                try:
                    ...
                    results[i] = (seg["start"], tmp)
                except Exception as e:
                    preview = seg["text"][:40].replace("\n", " ")
                    failed.append(i)
                    self._log(f"   ⚠️ Bỏ qua dòng {i+1}/{total_lines} (lỗi TTS): {preview}… ({e})")
            async with log_lock:
                done_count += 1
                ...
                if done_count % 5 == 0 or done_count == total_lines:
                    self._log(f"   🔊 TTS: đã xử lý {done_count}/{total_lines} dòng…")
                    self._set_stage(f"🔊 TTS {done_count}/{total_lines}", 40 + int(30 * done_count / total_lines))

        await asyncio.gather(*(_one(i, seg) for i, seg in enumerate(usable)))
        chunk_paths = [r for r in results if r is not None]  # giữ nguyên thứ tự start tăng dần
    ...
    def _process_one(self,fp,idx):
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as td:
                ...
                # ✅ TỐI ƯU: chỉ âm thanh bị đổi, HÌNH ẢNH không hề bị chỉnh sửa
                # gì cả -> copy thẳng luồng video ("-c:v copy") thay vì encode
                # lại toàn bộ video bằng libx264 như trước. Nhanh hơn RẤT
                # nhiều lần (thường chỉ vài giây thay vì vài phút/video) và
                # không hề mất chất lượng hình vì không nén lại.
                # Một số container/codec hiếm khi không copy được thẳng vào
                # .mp4 (vd HEVC nguồn lỗi, VP9...) -> nếu copy thất bại, tự
                # động encode lại như cách cũ (preset nhanh hơn) để đảm bảo
                # luôn ra được video.
                try:
                    run_ffmpeg([
                        "-i",fp,
                    ...
                    self._log(f"   ⚠️ Copy luồng video thất bại ({copy_err}) — encode lại (chậm hơn)…")
                    run_ffmpeg([
                        "-i",fp,
                        "-i",str(mp3),
                        "-filter_complex",filter_complex,
